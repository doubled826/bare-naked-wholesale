  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const statsData: DashboardStats = {
        ...
        todayRevenue: orders?.filter(o => new Date(o.created_at) >= today)
          .reduce((sum, o) => sum + (o.total || 0), 0) || 0,
        weekRevenue: orders?.filter(o => new Date(o.created_at) >= weekAgo)
          .reduce((sum, o) => sum + (o.total || 0), 0) || 0,
        monthRevenue: orders?.filter(o => new Date(o.created_at) >= monthAgo)
          .reduce((sum, o) => sum + (o.total || 0), 0) || 0,
      ...
      retailerOrders?.forEach((order: any) => {
        if (order.retailer) {
          ...
          const existing = retailerStats.get(key) || { 
      ...
      const topRetailersData = Array.from(retailerStats.entries())
        .map(([id, data]) => ({ id, ...data }))
    ...
    <div className="space-y-6">
            ...
            {topProducts.length === 0 ? (
              ...
              topProducts.map((product, index) => (
                <div key={product.id} className="p-4 flex items-center gap-4">
                  <div className="w-8 h-8 rounded-full bg-bark-100 flex items-center justify-center text-bark-600 font-bold text-sm">
                    {index + 1}
                  </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          ...
          <a href="/admin/orders" className="text-sm text-bark-500 hover:text-bark-600 font-medium">
            View all →
          </a>
        </div>
