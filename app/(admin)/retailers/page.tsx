...
export default function AdminRetailersPage() {
  ...
  const handleUpdateRetailer = async () => {
    ...
    try {
      ...
      showNotification('Retailer updated successfully!');
      setShowEditModal(false);
      setSelectedRetailer(null);
      fetchRetailers();
    } catch (error) {
      console.error('Error updating retailer:', error);
      showNotification('Failed to update retailer');
    } finally {
      setIsUpdating(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search retailers by name, account number, or address..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-bark-500 focus:border-transparent"
          />
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{retailers.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">
                {formatCurrency(retailers.reduce((sum, r) => sum + r.total_spent, 0))}
              </p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">
                {retailers.reduce((sum, r) => sum + r.total_orders, 0)}
              </p>
            </div>
          </div>
        </div>
                ...
                filteredRetailers.map((retailer) => (
                  <tr key={retailer.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-medium text-gray-900">{retailer.company_name}</p>
                        <p className="text-sm text-gray-500 truncate max-w-[200px]">{retailer.business_address}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="font-mono text-sm text-gray-600">{retailer.account_number}</span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {retailer.phone}
                    </td>
                    <td className="px-6 py-4">
                      <span className="font-medium text-gray-900">{retailer.total_orders}</span>
                    </td>
                    <td className="px-6 py-4 font-medium text-gray-900">
                      {formatCurrency(retailer.total_spent)}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">Retailer Details</h3>
              <button
                onClick={() => {
                  setSelectedRetailer(null);
                  setRetailerOrders([]);
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium text-gray-900">{selectedRetailer.company_name}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Account Number</p>
                  <p className="font-mono text-gray-900">{selectedRetailer.account_number}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">
                    {new Date(selectedRetailer.created_at).toLocaleDateString()}
                  </p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm text-gray-500">Business Address</p>
                  <p className="font-medium text-gray-900">{selectedRetailer.business_address}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-gray-100 pt-6">
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-2xl font-bold text-gray-900">{selectedRetailer.total_orders}</p>
                  <p className="text-sm text-gray-500">Total Orders</p>
                </div>
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-2xl font-bold text-gray-900">{formatCurrency(selectedRetailer.total_spent)}</p>
                  <p className="text-sm text-gray-500">Total Spent</p>
                </div>
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-2xl font-bold text-gray-900">
                    {selectedRetailer.total_orders > 0 
                      ? formatCurrency(selectedRetailer.total_spent / selectedRetailer.total_orders)
                      : '$0'}
                  </p>
              ...
              <div className="flex justify-end pt-4">
                <button
                  onClick={() => handleEditRetailer(selectedRetailer)}
                  className="px-4 py-2 bg-bark-500 text-white rounded-lg hover:bg-bark-600 transition-colors flex items-center gap-2"
                >
                  <Edit2 className="w-4 h-4" />
                  Edit Retailer
                </button>
