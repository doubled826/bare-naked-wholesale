  ...
  const filterOrders = () => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(order => 
        ...
        order.retailer?.company_name?.toLowerCase().includes(query)
      );
    }
    ...
    try {
      ...
      await fetch('/api/admin/orders/ship-notification', {
        ...
        body: JSON.stringify({
          ...
          retailerEmail: selectedOrder.retailer?.company_name // We'd need email from retailers
        })
      });
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex gap-2">
            <select
              ...
              className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-bark-500"
            >
              ...
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm">
                        ...
                        {order.order_items?.length > 2 && (
                          <p className="text-gray-400">+{order.order_items.length - 2} more</p>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      {order.tracking_number ? (
                        <span className="text-sm text-gray-600 font-mono">{order.tracking_number}</span>
                      ) : (
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="font-medium text-gray-900">{selectedOrder.retailer?.company_name}</p>
              </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="border-t border-gray-100 pt-4">
                <h4 className="font-medium text-gray-900 mb-3">Retailer Information</h4>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="font-medium text-gray-900">{selectedOrder.retailer?.company_name}</p>
                  <p className="text-sm text-gray-600 mt-1">{selectedOrder.retailer?.business_address}</p>
                  <p className="text-sm text-gray-600">{selectedOrder.retailer?.phone}</p>
                </div>
              </div>
                  ...
                  {selectedOrder.order_items?.map((item) => (
                    <div key={item.id} className="flex justify-between py-2 border-b border-gray-100 last:border-0">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{item.product?.size} × {item.quantity}</p>
                      </div>
